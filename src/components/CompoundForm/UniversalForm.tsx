...
const UniversalForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<DrugEntry>({
    marketInformation: {
      version: '',
      brandName: '',
      indication: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    try {
      ...
      const payload = processDrugData(formData);
      await postData(drugservice.createDrug(), payload);
      console.log('New Drug Entry:', payload);
