...
const CompoundDetailsPage: React.FC = () => {
  const { searchtext, cid } = useParams();
  

  const item = sampleDrugsData.find(
    (i) =>
      String(i.cid) === String(cid) &&
      (!searchtext ||
        i.MarketInformation.BrandName.toLowerCase().includes(
...
console.log(item)
  if (!item)
    return (
      <div className="p-8 text-center text-red-600 text-xl">
        No data found for this compound with CID {cid} and search text "{searchtext}".
      </div>
    );
...
// const renderTOC=(item:any)=>{
//   return (<div>
//     {Object.keys(item).map((key)=>{
//       const value=item[key];
//       if(typeof value==="object" && value!==null){
//         return (
//           <div key={key} className="ml-4 pl-4 border-l-2 border-gray-300 mb-2">
//             <h3 className="font-bold text-lg mb-1">{key}</h3>
//             {renderTOC(value)}
//           </div>
//         )
//       }else{
...
console.log(value)
        ...
        if (value && typeof value === "object" && !Array.isArray(value)) {
          const isPlainObject = Object.keys(value).every(
          ...
          if (isPlainObject) {
                  ...
                  <tbody>
                    {Object.keys(value).map((subKey) => (
                      <tr key={subKey} className="border-b border-gray-200">
                        <td className="px-2 py-1 font-semibold">{subKey}</td>
                        <td className="px-2 py-1">{String(value[subKey])}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            return (
              <div
                key={key}
                className="ml-4 pl-4 border-l-2 border-gray-300 mb-2"
              >
                <h3 className="font-bold text-lg mb-1">{key}</h3>
                {renderTOC(value)}
              </div>
          ...
          // If value is array, map recursively
          return (
            <div key={key} className="mb-2">
              <h3 className="font-bold text-lg mb-1">{key}</h3>
              {value.map((v, i) => (
                <div key={i} className="ml-4">
                  {typeof v === "object" ? renderTOC(v) : String(v)}
                </div>
              ))}
            </div>
          );
  ...
  return (
    <div className="p-8">
     <div>
      <h1>{renderTOC(item)}</h1>
     <LeftPanel item={item} cid={cid} />
     </div>
     <div></div>
    </div>
  );
