...
const ResultList: React.FC = () => {
  ...
  // Filter results based on brand name / generic name / synonyms
  const results = sampleDrugsData.filter((item: any) => {
    const search = (searchtext || "").toLowerCase();
    if (!search) return false;

    const brand = item?.MarketInformation?.BrandName?.toLowerCase() || "";
    const generic = item?.MarketInformation?.GenericName?.toLowerCase() || "";
    const synonyms = (item?.MarketInformation?.Synonyms || [])
...
//console.log(item)
    return (
      ...
      synonyms.includes(search)
    );
  });



  // Keep search text in SearchBar
  const [searchValue, setSearchValue] = useState(searchtext || "");
  useEffect(() => {
    setSearchValue(searchtext || "");
  }, [searchtext]);

  const handleNavigate = (brandName: string,cid:string) => {
    if (brandName&&cid) {
      navigate(`/drug/${encodeURIComponent(brandName)}/${cid}`);
    }
  };
  ...
  return (
    <div className="w-full min-h-[60vh] flex flex-col items-center bg-white/80 py-8">
      <div className="w-full max-w-5xl">
        ...
        <div className="flex items-center gap-8 border-b pb-2 mb-4 mt-4">
          <span className="font-bold text-blue-900 text-lg">
            {results.length} results
          </span>
          <button className="ml-auto px-4 py-1 rounded bg-gray-100 border text-gray-700">
            Filters
          </button>
          <select className="px-2 py-1 border rounded bg-white text-gray-700">
            <option>Sort by Relevance</option>
        ...
        {/* Results list */}
        <div className="space-y-6 w-full md:w-2/3">
          {results.map((drug: any, idx: number) => {
            const cid=drug.cid
            const brand = drug?.MarketInformation?.BrandName || "Unknown";
            const indication = drug?.MarketInformation?.Indication || "";
            const generic = drug?.MarketInformation?.GenericName || "";
            const approvedDate = drug?.MarketInformation?.ApprovedDate || "";
            const approvedCountries=drug?.MarketInformation?.ApprovedCountries.join(",")||"";
            const elementFormula = drug?.DrugSubstance?.PhysicalAndChemicalProperties?.ElementalFormula || "";
            const molecularWeight = drug?.DrugSubstance?.PhysicalAndChemicalProperties?.MolecularWeight || "";
            const iupac = drug?.DrugSubstance?.PhysicalAndChemicalProperties?.IUPACName || "";
            const chemicalSturucture=drug?.DrugSubstance?.PhysicalAndChemicalProperties?.ChemicalStructure||"";
            //const synonyms = drug?.MarketInformation?.Synonyms?.join("; ") || "";
            const testMethods=drug?.DrugProduct?.Formulation?.TestMethods||"";
            console.log("iupac",cid)
            return (
              <div
                key={idx}
                className="flex flex-col md:flex-row border p-4 rounded shadow mb-4 bg-white w-[1000px] overflow-x-auto"
              >
                ...
                <div className="mr-6 flex flex-col items-start min-w-[220px] max-w-[400px]">
                  <img
                    src="https://via.placeholder.com/150?text=Drug+Image"
                    alt={chemicalSturucture}
                    className="h-48 w-full max-w-[380px] object-contain"
                  />
                </div>
                ...
                <div className="flex flex-col space-y-2 text-left min-w-0 break-words w-full justify-start">
                  <p
                    className="text-md text-blue-600 font-semibold cursor-pointer hover:underline"
                    onClick={() => handleNavigate(brand,cid)}
                  >
                    {brand};{iupac}
                  </p>
                  {generic && (
                    <p className="text-gray-800">
                      ...
                      <span className="text-black">{generic}</span>
                    </p>
                  )}
                  {indication && (
                    <p className="text-gray-800">
                      <strong>Indication:</strong>{" "}
                      <span className="text-black">{indication}</span>
                    </p>
                  )}
                  ...
                  {elementFormula && (
                    <p className="text-gray-800">
                      ...
                      <span className="text-black">{elementFormula}</span>
                    </p>
                  )}
                  {molecularWeight && (
                    <p className="text-gray-800">
                      <strong>Molecular Weight:</strong>{" "}
                      <span className="text-black">{molecularWeight} g/mol</span>
                    </p>
                  )}
                  {iupac && (
                    <p className="text-gray-800">
                      <strong>IUPAC Name:</strong>{" "}
                      <span className="text-black">{iupac}</span>
                    </p>
                  )}
                  {testMethods && (
                    <p className="text-gray-800">
                      <strong>Test Methods:</strong>{" "}
                      <span className="text-black">{testMethods}</span>
                    </p>
                  )}
                  {approvedCountries && (
                    <p className="text-gray-800">
                      <strong>Approved Countries:</strong>{" "}
                      <span className="text-black">{approvedCountries}</span>
                    </p>
                  )}
                  {approvedDate && (
                    <p className="text-gray-800">
                      <strong>Approved Date:</strong>{" "}
                      <span className="text-black">{approvedDate}</span>
                    </p>
                  )}
                </div>
              </div>
